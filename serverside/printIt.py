...
def fetchImageWithUrl(url):
	...
	return img'''	
	req = urllib2.Request(url)
	regErr = urllib2.Request("http://tc0.loftmyndir.is/tc_r/tilecache.py/lightsaber/2/0/3.jpeg")
	# Make a temporary file without saving it on the harddrive.
	try:
		f = cStringIO.StringIO(urllib2.urlopen(req).read())
	except:
		f = cStringIO.StringIO(urllib2.urlopen(regErr).read()) # reads blank blue tile #return img
	img = Image.open(f)
	return img
...
def main():
	...
	#maxWidth = 800
	#resizeHeight = maxWidth/1.9178
	#img = img.resize( (maxWidth, int(resizeHeight)), Image.ANTIALIAS)
		
	img.save(outputPath, quality=100)
	...
	if ext=="PDF":
		...
		toPDF.makePdf(outputPath, pdfOutputPath, "www.map.is")
...
if __name__=='__main__':
	...
	#cgitb.enable(display=0, logdir="\tmp")
	main()
