...
function retrievedGeoJSON(resp)
{
	...
	if(xmlHttp.readyState == 4)
	{
		...
		log("Number of features: "+features.length);
		log("Databits count: " + rData.length)
		...
		for(var j=0; j<rData.length; ++j)
		{
			...
			else // rest
			{
				...
				if (Math.abs(a) <= 10)
					turnObj.turnTxt = "haltu áfram";

				if ( a > 10 )
					turnObj.turnTxt = "beygðu til vinstri";

				if ( a < -10 )
					turnObj.turnTxt = "beygðu til hægri";
...
function clientConfig()
{
	...
	// Lets play with the right click! Weeee
	/*map.div.oncontextmenu = function noContextMenu(e) {
        if (OpenLayers.Event.isRightClick(e))
		{
			//var rightClickMenuHTML = "<div id=rightClickMenu style=position></div>";
			alert("vvv"); // Add the right click menu here
        }
