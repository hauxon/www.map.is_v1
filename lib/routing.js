...
// Setjum inn vectorlayer til að birta niðurstöðu vegvísunar á kortinu
 var vectors = new OpenLayers.Layer.Vector("Vegvísun", {style: style_routing, 'displayInLayerSwitcher':true});
...
function getRoute()
{
 ...
 function checkAddressAndZipUnique(sValue1, sValue2)
 {
	...
	//sendSyncAJAXRequest("proxies/leit_proxy.asp?sKey=checkAddressAndZipUnique&sValue=" + sValue1+"|"+sValue2);
        sendSyncAJAXRequest("db/routingDB.php?sKey=checkAddressAndZipUnique&sValue=" + sValue1+"|"+sValue2);
...
{
	// Jæja sækja leiðina
	sendAJAXRequest('proxies/routing_proxy.asp?request=route&to_addr=' + escape(addrTo) + '&to_city=' + zipTo + '&from_addr=' + escape(addrFrom) + '&from_city=' + zipFrom + '',displayResultsGeoJSON);
}

// Sends an AJAX request to routing_proxy_click with coordinates (routeFromHere/routeToHere click)
function getRoutePathClick()
{
	if (routeFromHereCoords != "" && routeToHereCoords != "")
	{
		...
		vectors.destroyFeatures();
		// Jæja sækja leiðina
		sendAJAXRequest('proxies/routing_proxy_click.asp?request=route&xfrom=' + routeFromHereCoords.lon + '&yfrom=' + routeFromHereCoords.lat + '&xto=' + routeToHereCoords.lon + '&yto=' + routeToHereCoords.lat + '', displayResultsGeoJSON);
	}
}
